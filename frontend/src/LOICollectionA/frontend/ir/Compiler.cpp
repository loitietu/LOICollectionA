...
namespace LOICollection::frontend::ir {
    ...
    void Compiler::visit(ImportNode&) {
        /* Imports are resolved and merged by the loader before compilation;
         * the compiler simply skips the import statement. */
    }
    ...
    void Compiler::compileDeclarativeBlock(BlockNode& block, const std::string& formClass, const SourceLocation& loc) {
        ...
        std::string formVar = "__form_" + std::to_string(this->declBlockCounter++);
        int formIdx = this->addConstant(formVar);
        this->current.get().emit(OpCode::DECLARE_LOCAL, formIdx, loc);
        this->current.get().emit(OpCode::STORE_VAR, formIdx, loc);
        ...
        for (auto& part : block.parts) {
            ...
            if (producesValue)
                this->current.get().emit(OpCode::POP, 0, loc);
        }
    ...
    void Compiler::visit(FuncCallNode& node) {
        if (node.implicitReceiver) {
            if (!this->formClass || !this->formVarIdx ||
                !ClassCall::getInstance().isRegistered(*this->formClass)) {
                this->diagnostics.addError(node.loc,
                    "Implicit call is only valid inside a declarative UI block");
                return;
            }

            /* Arguments first, then the form receiver on top (native call order);
             * the form itself stays in the frame-local for later statements. */
            for (auto& arg : node.args)
                this->compileValue(*arg, node.loc);
            ...
            int argCount = static_cast<int>(node.args.size());
            int metaIdx = this->addNativeCall(*this->formClass, node.name, argCount);
