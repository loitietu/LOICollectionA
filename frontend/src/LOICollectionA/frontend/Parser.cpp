...
namespace LOICollection::frontend {
    ...
    std::unique_ptr<ImportNode> Parser::parseImport() {
        ...
        if (currentToken.type != TokenType::TOKEN_STRING) {
            diagnostics.addError(currentToken.loc, "Expected import path string");
            synchronize({ TokenType::TOKEN_SEMICOLON, TokenType::TOKEN_EOF });
            return nullptr;
        }

        auto importNode = std::make_unique<ImportNode>(loc, currentToken.value);
        if (!eat(TokenType::TOKEN_STRING)) return nullptr;
    ...
    std::unique_ptr<BlockNode> Parser::parseDeclarativeBlock() {
        if (!eat(TokenType::TOKEN_LBRACE)) return nullptr;

        auto block = std::make_unique<BlockNode>();

        bool saved = this->inDeclarativeBlock;
        this->inDeclarativeBlock = true;
    ...
    std::vector<std::unique_ptr<ExprNode>> Parser::parseArgs(TokenType delimiterToken, TokenType stopToken) {
        ...
        /* `on:` named-argument sugar, recognised only inside declarative UI blocks.
         * It fills the callback slot, i.e. it is inserted before the trailing options arg. */
        std::unique_ptr<ExprNode> onArg;
        ...
        while (currentToken.type != stopToken &&
            ...
            if (this->inDeclarativeBlock &&
                currentToken.type == TokenType::TOKEN_IDENT && currentToken.value == "on" &&
                peek() == TokenType::TOKEN_COLON) {
                if (!eat(TokenType::TOKEN_IDENT)) break;
                if (!eat(TokenType::TOKEN_COLON)) break;

                onArg = parseBaseExpression();
                if (!onArg) {
        ...
        if (onArg) {
            if (args.size() > 1)
                args.insert(args.begin() + 1, std::move(onArg));
            else
                args.push_back(std::move(onArg));
        }
    ...
    std::unique_ptr<ExprNode> Parser::parsePrimary() {
        /* Only the first primary of a declarative-block statement may be an
         * implicit call on the form being constructed. */
        bool atStmtStart = this->declStmtStart;
        this->declStmtStart = false;
        ...
        switch (currentToken.type) {
            ...
            case TokenType::TOKEN_NEW: {
                ...
                if (currentToken.type == TokenType::TOKEN_LBRACE)
                    node->declarativeBlock = this->parseDeclarativeBlock();
            ...
            case TokenType::TOKEN_IDENT: {
                ...
                if (peek() == TokenType::TOKEN_LPAREN) {
                    ...
                    auto node = std::make_unique<FuncCallNode>(loc, std::move(name), std::move(args));
                    node->implicitReceiver = atStmtStart;
                    return node;
                }
