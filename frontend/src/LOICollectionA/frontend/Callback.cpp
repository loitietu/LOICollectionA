...
namespace LOICollection::frontend {
   ...
   Expected<TypedValue> FunctionCall::callFunction(const std::string& namespaces, const std::string& function, const CallbackTypeValues& args, const CallbackTypePlaces& placeholders, DiagnosticEngine& diagnostics, const SourceLocation& loc) {
        ...
        if (this->mImpl->mFunctions[namespaces].find(sig) != this->mImpl->mFunctions[namespaces].end()) {
            ...
            if (!result.has_value())
                return makeStringError("Function callback threw: " + result.error().message());
    ...
    Expected<TypedValue> MacroCall::callMacro(const std::string& name, const CallbackTypeValues& args, const CallbackTypePlaces& placeholders, DiagnosticEngine& diagnostics, const SourceLocation& loc) {
        std::vector<ParamType> argTypes = valuesToTypes(args, diagnostics, loc);
        ...
        if (this->mImpl->mMacros.find(sig) != this->mImpl->mMacros.end()) {
            ...
            if (!result.has_value())
                return makeStringError("Macro callback threw: " + result.error().message());
    ...
    Expected<TypedValue> ClassCall::getStaticField(const std::string& className, const std::string& field) const {
        auto it = this->mImpl->classes.find(className);
        if (it == this->mImpl->classes.end())
            return makeErrorCodeError(std::make_error_code(std::errc::invalid_argument));
    ...
    Expected<ObjectRef> ClassCall::create(
        ...
        if (info.constructors.find(sig) != info.constructors.end()) {
            ...
            if (!result.has_value())
                return makeStringError("Constructor callback threw: " + result.error().message());
    ...
    Expected<TypedValue> ClassCall::callMethod(
        ...
        if (info.methods.find(sig) != info.methods.end()) {
            ...
            if (!result.has_value()) 
                return makeStringError("Method callback threw: " + result.error().message());
    ...
    Expected<TypedValue> ClassCall::callStaticMethod(
        const std::string& className, const std::string& method, const CallbackTypeValues& args,
        ...
        if (info.staticMethods.find(sig) != info.staticMethods.end()) {
            ...
            if (!result.has_value())
                return makeStringError("Static method callback threw: " + result.error().message());
    ...
    Expected<TypedValue> ClassCall::callValueMethod(
        ...
        if (!result.has_value())
            return makeStringError("Value method callback threw: " + result.error().message());

        return result;
    }

    Expected<TypedValue> ClassCall::callOperator(
        ...
        if (!result.has_value())
            return makeStringError("Operator callback threw: " + result.error().message());
