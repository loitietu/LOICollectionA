...
namespace LOICollection::frontend {
    ...
    private:
        struct MethodScope {
            ...
            /* Class of the form currently being constructed in a declarative
            ...
            std::vector<std::unordered_set<std::string>> blockPreExisting;
            ...
            void enterBlockScope(const std::unordered_map<std::string, TypeInfo>& globals,
                                 const std::unordered_map<std::string, TypeInfo>& declared) {
                std::unordered_set<std::string> existing;
                for (const auto& [name, _] : globals) existing.insert(name);
                for (const auto& [name, _] : declared) existing.insert(name);

                this->blockPreExisting.push_back(std::move(existing));
            ...
            void declareLocal(const std::string& name) {
                ...
                auto& existing = this->blockPreExisting.back();
                if (!existing.contains(name))
                    this->blockScopes.back().insert(name);
            }

            /* Returns the names newly declared by the innermost block scope;
             * these must be removed from the global tables when the scope exits. */
            [[nodiscard]] std::vector<std::string> exitBlockScope() {
                std::vector<std::string> names;
                if (!this->blockScopes.empty()) {
                    names.assign(this->blockScopes.back().begin(), this->blockScopes.back().end());
                    this->blockScopes.pop_back();
                    this->blockPreExisting.pop_back();
                }
                return names;
            }

            [[nodiscard]] bool inBlockScope() const { return !this->blockScopes.empty(); }
        };
        ...
        [[nodiscard]] static bool isDeclarativeFormClass(const std::string& name);
