...
TEST(ArrayTest, IndexRead) {
    ...
    EXPECT_EQ(eval("let a = [1, 2, 3]; a[0] + a[2]"), "4");
}

TEST(ArrayTest, IndexWrite) {
    EXPECT_EQ(eval("let a = [1, 2]; a[0] = 9; a[0]"), "9");
}

TEST(ArrayTest, AppendAtIndexLength) {
    EXPECT_EQ(eval("let a = [1]; a[1] = 2; a.length()"), "2");
    EXPECT_EQ(eval("let a = []; a[0] = 5; a[0]"), "5");
}

TEST(ArrayTest, Length) {
    EXPECT_EQ(eval("[1, 2, 3].length()"), "3");
    EXPECT_EQ(eval("let a = []; a.length()"), "0");
}

TEST(ArrayTest, NestedArrays) {
    EXPECT_EQ(eval("let a = [[1, 2], [3, 4]]; a[1][0]"), "3");
}

TEST(ArrayTest, MixedTypes) {
    EXPECT_EQ(eval("let a = [1, \"x\", true]; a[1]"), "x");
}

TEST(ArrayTest, IdentityEquality) {
    EXPECT_EQ(eval("let a = [1]; a == a"), "true");
    EXPECT_EQ(eval("let a = [1]; let b = [1]; a == b"), "false");
    EXPECT_EQ(eval("let a = [1]; let b = a; a == b"), "true");
}
...
TEST(ArrayTest, IndexOutOfRange) {
    ...
    EXPECT_THROW(eval("let a = [1]; a[-1]"), std::runtime_error);
}

TEST(ArrayTest, IndexNonArray) {
    EXPECT_THROW(eval("let a = 5; a[0]"), std::runtime_error);
}
