...
> [!NOTE]
> 以下内容取自 LOICollectionA 1.17.0 的 `.lcui` 脚本语言实现，对于后续版本可能会有所不同。
...
- 插件启动时或执行 `/xxx reload` 命令后，`GUIManager::load` 会读取 `.lcui` 文件，先解析 `import` 导入图（见「多文件导入」），再完成上述编译流程；编译产物除缓存在内存外，还会以 `.lcp` 字节码文件落盘（见「字节码产物」）。
...
未标注类型的 `let` 声明即为动态类型，可随时赋其他类型的值：
...
字符串也支持以 `[]` 取下标索引，返回指定位置的单个字符（类型为 `string`），越界或负索引会报错：
...
能否遍历由**迭代协议**决定，而不是编译器逐个类型特判：

| 可迭代对象 | 元素 | 下标 |
| --- | --- | --- |
| 数组 | 元素值 | 位置（`int`） |
| 字符串 | 单个字符，类型为 `string` | 位置（`int`） |
...
`GUIManager::load` 编译脚本后，会把字节码序列化到源文件旁边的 `<脚本名>.lcp` 文件（例如 `market.lcui` 对应 `market.lcui.lcp`）：

- 再次加载时优先读取 `.lcp` 产物，命中则跳过词法/语法/语义分析与编译，直接复用字节码；
- 产物头记录脚本 id 与 **ABI 指纹**（所有已注册原生类、函数、宏的签名形状哈希）；插件升级导致任一原生签名变化时，指纹不匹配会让旧产物自动失效并重新编译；
- 源文件在场时，产物还会记录入口文件与整个导入图中每个文件的 SHA-256，任一文件内容变化都会重新编译；
- 序列化数据附带校验和，损坏的产物会被丢弃并重新编译；
