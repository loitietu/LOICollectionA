...
### 纯函数折叠：把 `math::abs(-3)` 也算掉

栈上的值全是常量时，连原生函数调用都能提前算。优化器认得 `math::` 命名空间里那批无副作用的纯函数：`abs` / `min` / `max` / `sqrt` / `log` / `sin` / `cos` / `pow`。只要参数全部是已知常量、且参数类型恰好命中注册表里的某个签名，调用指令连同参数一起被替换成一条 `PUSH`。

边界同样清晰：`math::random()` 是非纯函数，永远不折；`math::abs(-2147483648)` 会触发运行期溢出错误，不折——让错误照常发生；参数不是常量，更不折。和算术折叠共用一条铁律：**折叠结果必须和运行期调用一模一样，包括错误**。
...
还有一类"盯着一小段指令模式改写"的优化叫**窥孔优化**（peephole optimization）。最典型的一种：条件跳转后面紧跟一条跳向同一目标的无条件跳转——不管条件真假都会落到同一个地方，那这个条件还有什么用？优化器把两条跳转换成一条 `POP`（条件值还是得弹掉，栈要平），夹在中间的代码变成不可达，交给可达性分析收拾。
...
| 跳转窥孔 | 条件跳转 + 同目标无条件跳转 | 双跳转合并为 `POP`，中间代码不可达 |
| 超级指令 | `DUP; STORE_VAR`、`DUP; IS_NONE` | 融合为 `DUP_STORE` / `DUP_IS_NONE`，省一次分发 |
| 不折叠 | `10.0 % 3.0`、空 Optional 的 `UNWRAP`、`math::abs(INT_MIN)` | 折叠会吞掉运行期错误，必须保留 |
...
graph TD
    ...
    Pre --> Sim[虚拟栈模拟: 折叠 / 变量转发 / 纯函数求值 / 指令融合]
    ...
    Dead --> Changed{本轮有改动?}
    Changed -->|是| Sim
    Changed -->|否| Patch[回填最终跳转偏移]
    Patch --> Out[输出优化后的 BytecodeChunk]
```

当然它也有明确的"不做清单"：没有公共子表达式消除（CSE），没有强度削减，循环不变量外提这类依赖完整数据流分析的深度优化也一概没有。栈模拟器是线性扫描，见到跳转目标就清空已知状态——它赚的是"顺路"的钱，不为此重建整个控制流图。
...
这条路的固定成本不低：每次调用都要把参数逐个转成类型向量、算出签名哈希、查两层 map（先命名空间后签名）才能摸到回调。一次调用无所谓，但 `while` 循环里跑几百轮的 `math::abs(...)`、`arr.push(...)` 就不一样了——每一轮都在重复同一套查找。

VM 的对策是**单态内联缓存**（monomorphic inline cache）：每个原生调用点配一个缓存槽，记住上一次的解析结果——函数名、参数类型列表、回调本体。再次执行到同一个调用点时，先花一次轻量的逐参数类型比对，比对通过就直接调用缓存的回调，签名哈希和两层 map 查找全省。这个思路和 CPU 的分支预测是一个味道：赌"这一次和上一次是同一种调用"。赌对了近乎免费，赌错了也只是回退到完整查找重新解析，正确性由比对逻辑兜底。

缓存什么时候失效？两类情况：一是参数形状变了（上一轮传 `int`、这一轮传 `float`，命中的是不同重载），缓存槽直接换血；二是注册表本身动了——任何 `registerFunction` / `registerMethod` 都会递增一个全局 `epoch` 计数，槽里记着的 epoch 对不上就作废。所以插件热重载、运行期动态注册都不会让陈旧的回调被错误地调用。至于同一个调用点交替调用不同类的方法（`a.put(); b.put(); a.put(); ...`）这种"缓存颠簸"，代价也只是每次都 miss 走慢路径，绝不比没有缓存更差。

顺带一提，这也是为什么"同一个对象的连续调用"（`a.put(); a.put(); a.print();`）天然比"a、b 交替调用"更讨缓存喜欢——前者的每一次调用都能命中上一个槽位，后者每次都得换血。不过这只是性能层面的甜头，写脚本时大可不必为它调整结构。
...
| 超级指令 | `DUP_STORE` | 名字常量下标 | `... v → ... v` | `DUP; STORE_VAR` 的融合：保留栈顶同时存入变量 |
| | `DUP_IS_NONE` | — | `... v → ... v, bool` | `DUP; IS_NONE` 的融合：保留栈顶同时压入判空结果 |
...
DUP_STORE a            ... 42        → ... 42       （DUP + STORE_VAR 的融合形态）
