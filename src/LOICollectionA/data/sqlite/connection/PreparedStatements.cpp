...
namespace {
    ...
    constexpr auto kCatalog = std::to_array<StatementSpec>({
        ...
        { "getBlockByName",
            "SELECT id,parent,name,kind,state,created,updated,payload FROM block WHERE parent=? AND name=? AND state < 8 LIMIT 1" },
        { "getIdByName", "SELECT id FROM block WHERE parent=? AND name=? AND state < 8 LIMIT 1" },
        { "updateState",
            "UPDATE block SET state=?,updated=? WHERE id=?" },
        { "setPayload",
            "UPDATE block SET payload=?,updated=? WHERE id=?" },
        { "listChildren",
            "SELECT id FROM block WHERE parent=? AND state < 8 ORDER BY id LIMIT ?" },
        { "listChildrenKind",
            "SELECT id FROM block WHERE parent=? AND kind=? AND state < 8 ORDER BY id LIMIT ?" },
        { "getChildrenFull",
            "SELECT id,parent,name,kind,state,created,updated,payload FROM block "
            "WHERE parent=? AND state < 8 ORDER BY id LIMIT ?" },
        { "getChildrenFullKind",
            "SELECT id,parent,name,kind,state,created,updated,payload FROM block "
            "WHERE parent=? AND kind=? AND state < 8 ORDER BY id LIMIT ?" },
        { "listChildNames",
            "SELECT id,name FROM block WHERE parent=? AND state < 8 ORDER BY id" },
        { "getState", "SELECT state FROM block WHERE id=?" },
        { "countChildren",
            "SELECT COUNT(*) FROM block WHERE parent=? AND state < 8" },
        { "countChildrenKind",
            "SELECT COUNT(*) FROM block WHERE parent=? AND kind=? AND state < 8" },
        ...
        { "queryPropText",
            ...
            "WHERE p.key=? AND p.tval=? AND b.state < 8 ORDER BY b.id LIMIT ?" },
        { "queryPropInt",
            "SELECT DISTINCT b.id FROM block b JOIN prop p ON p.block_id=b.id "
            "WHERE p.key=? AND p.ival>=? AND p.ival<=? AND b.state < 8 ORDER BY b.id LIMIT ?" },
        { "queryPropIntUnder",
            "SELECT DISTINCT b.id FROM block b JOIN prop p ON p.block_id=b.id "
            "WHERE p.key=? AND p.ival>=? AND p.ival<=? AND b.parent=? AND b.state < 8 ORDER BY b.id LIMIT ?" },
        { "queryPropTextUnder",
            "SELECT DISTINCT b.id FROM block b JOIN prop p ON p.block_id=b.id "
            "WHERE p.key=? AND p.tval=? AND b.parent=? AND b.state < 8 ORDER BY b.id LIMIT ?" },
