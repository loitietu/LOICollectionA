...
namespace {
    ...
    constexpr auto kCatalog = std::to_array<StatementSpec>({
        ...
        { "listChildren",
            "SELECT id FROM block WHERE parent=? AND (state & 24)=0 ORDER BY id LIMIT ?" },
        { "listChildrenKind",
            "SELECT id FROM block WHERE parent=? AND kind=? AND (state & 24)=0 ORDER BY id LIMIT ?" },
        { "getChildrenFull",
            "SELECT id,parent,name,kind,state,created,updated,payload FROM block "
            "WHERE parent=? AND (state & 24)=0 ORDER BY id LIMIT ?" },
        { "getChildrenFullKind",
            "SELECT id,parent,name,kind,state,created,updated,payload FROM block "
            "WHERE parent=? AND kind=? AND (state & 24)=0 ORDER BY id LIMIT ?" },
        { "listChildNames",
            "SELECT id,name FROM block WHERE parent=? AND (state & 24)=0 ORDER BY id" },
        { "getState", "SELECT state FROM block WHERE id=?" },
        { "countChildren",
            "SELECT COUNT(*) FROM block WHERE parent=? AND (state & 24)=0" },
        { "countChildrenKind",
            "SELECT COUNT(*) FROM block WHERE parent=? AND kind=? AND (state & 24)=0" },
